define(function(require) {
	...
	(function() {
		...
		var dateStar = moment().format('YYYY-MM-DD'),
			dateEnd = moment().day(-1).format('YYYY-MM-DD');
		var res, parameter;
		var lineCost;
		...
		function getReportAll(data) {
			...
			$.each(data.status.data.piegraghES, function(i, v) {
				item = new Object;
				item.name = v.classinstancename + "<br>" + v.singleClasspropertyname + v.singleDatavalue + "kWh";
				item.y = 0;
		...
		$('#myModal').on('shown.bs.modal', function() {
			...
			if ($('input:radio[name="group"]:checked').val() == 'glj') {
				$(".qtpage").hide();
				$(".qt-gongleng").show();
			} else {
				$(".qtpage").hide();
			...
			if (area == 0) {
				...
				//显示 c 冷温水输配子系统 table1
				formatTables("d_t_01", ["设备序号", "耗电量", "输送系数"], tablesCWCP);
				//显示 c 冷却子系统 table1
